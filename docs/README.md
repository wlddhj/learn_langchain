...
LangChain 是一个用于构建基于大语言模型（LLM）应用的开源框架。它提供了一套标准化的组件和接口，让开发者能够将 LLM 与外部数据源、工具、记忆系统等能力组合在一起，构建出复杂的 AI 应用。

### 核心能力

- **模型调用**：统一的接口对接 OpenAI、智谱 AI、DeepSeek、通义千问等多种 LLM 提供商
- **Prompt 工程**：模板化管理提示词，支持变量注入、Few-shot 示例、对话历史
- **链式编排（Chain）**：通过 LCEL（LangChain Expression Language）将多个组件像管道一样串联
- **检索增强生成（RAG）**：连接私有数据源，让 LLM 基于文档、数据库等内容生成回答
- **工具与 Agent**：赋予 LLM 调用外部工具（API、数据库、文件系统）的能力，实现自主决策
- **记忆管理（Memory）**：在多轮对话中维护上下文，支持缓冲、摘要、窗口等多种策略
- **可观测性**：集成 LangSmith 实现链路追踪、调试和性能监控

### 适用场景

智能客服、文档问答、代码审查、数据分析、自动化工作流、多 Agent 协作等。
