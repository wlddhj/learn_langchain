...
> 📖 **配套阅读**：如需深入理解底层原理，请阅读 [概念深度解析](26-concepts-deep-dive.md)。
>
> 该文档详细解释：为什么微调只需 0.1% 参数、低秩分解的数学直觉、量化机制、AdaLoRA 动态调整等。
...
> 💡 **核心直觉**：重新训练整个模型——所有 70 亿参数都参与更新。
>
> 🎨 **类比**：把整本书重写一遍，而不是只修改几个章节。成本最高，但定制程度也最高。
...
>
> 🎨 **类比**：给一幅画调色，不需要重新画整幅，只需调整几个关键色调，画面气质就完全不同了。
...
> 💡 **核心直觉**：神经网络权重有大量"冗余精度"——16位存储的参数，其实很多位是"噪声"，真正有意义的信息只需要4位就能表达。
>
> 🎨 **类比**：一张高清照片（16-bit）压缩成缩略图（4-bit），虽然细节丢失，但主要内容依然清晰可辨。
...
> 💡 **核心直觉**：在模型的"高速公路"旁修建"小岔路"——原模型主体不动，只在关键节点插入小型可训练模块。
>
> 🎨 **类比**：城市主干道（原模型）保持不变，只在路口加装小型调节站（Adapter），就能改变整体交通流向。切换 Adapter 就像切换不同的调度方案。
...
> 💡 **核心直觉**：不同层的"重要性"不同——有些层微调时需要更多参数，有些层几乎不需要改动。AdaLoRA 会自动判断哪些层需要更多"调整空间"。
>
> 🎨 **类比**：装修房子时，客厅需要大改（高秩），卫生间只需小调（低秩）。AdaLoRA 就像智能设计师，自动分配每个房间的装修预算。

**原理**：动态调整各层的 LoRA 秩（rank）。

```
核心思想：
├── LoRA 固定 rank=16，所有层使用相同秩
├── AdaLoRA 让重要层使用更高秩（如 32）
├── 不重要层使用更低秩（如 4）甚至降为 0
├── 自动学习每层的"重要性权重"
└── 结果：更高效的参数分配，效果相近但参数更少

为什么需要动态？
├── Transformer 不同层的作用不同
│   ├── 底层：处理基础语法、词法
│   ├── 中层：处理语义、句法
│   └顶层：处理任务特定知识
├── 微调时，顶层通常需要更多调整
...
> 💡 **核心直觉**：不改模型"大脑"，只改"输入引导"——在输入前添加一组可学习的"软向量"，引导模型输出特定风格的内容。
>
> 🎨 **类比**：老师（模型）的知识不变，但课前给学生发一套特定的"引导卡片"（软提示），老师看到卡片后会按特定方式回答。
